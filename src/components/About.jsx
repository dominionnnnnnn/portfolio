...
const About = () => {
  ...
  return (
    <div id='about'  className='scroll-mt-20 scroll-smooth my-4 py-4 px-4 md:px-10 lg:px-17'>
      <header ref={headerRef} className={`md:text-4xl text-3xl text-gray-300 tracking-wide  my-2 border-t-1 border-gray-500 pt-6 font-light about-header ${isHeaderVisible ? "show" : ""}`}>About me</header>
      <div className='flex flex-wrap md:flex-nowrap justify-center gap-12 lg:gap-24 mt-4'>
    <div>
      <img ref={imgRef} src={img} alt="" className={`h-120 md:h-100 lg:h-120 about-img ${isImgVisible ? "show" : ""}`} />
    </div>
    <div>
      <header ref={headerTextRef} className={`text-white text-4xl lg:text-6xl font-bold lg:font-semibold headertext name lg:tracking-wide lg:my-4 leading-14 lg:leading-18 ${isHeaderTextVisible ? "show" : ""} `}>Hi, I'm <br /> Adebiyi <br /> AbdulQuawiy</header>
      <div ref={bottomTextRef} className={`flex gap-6 lg:mt-12 bottomtext ${isBottomTextVisible ? "show" : ""}`}>
